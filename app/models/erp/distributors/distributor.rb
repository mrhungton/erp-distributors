      ...
      def self.filter(query, params)
        ...
        if params[:keyword].present?
          ...
          keyword.split(' ').each do |q|
            q = q.strip
            query = query.where('LOWER(erp_distributors_distributors.name) LIKE ? OR LOWER(erp_distributors_distributors.name) LIKE ? OR LOWER(erp_distributors_distributors.name) LIKE ?', q+'%', '% '+q+'%', '%-'+q+'%')
          end
        end
